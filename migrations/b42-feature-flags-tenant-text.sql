-- B42: ALTER feature_flag tables tenant_id UUID → TEXT
-- Same pattern as B38 fix for cuts/customer_payments/etc.

DO $$
DECLARE r RECORD;
BEGIN
  FOREACH r.tablename IN ARRAY ARRAY[
...
-- Drop policies one-by-one explicit
DO $$
DECLARE r RECORD;
BEGIN
  FOR r IN
...
-- Recreate policies (tenant isolation by JWT)
