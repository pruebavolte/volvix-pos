...
(async () => {
  ...
  for (const b of ['mezcla','burbuja']) {
    const page = await browser.newPage();
    const errors = [];
    page.on('pageerror', e => errors.push(e.message + ' @line ' + (e.stack ? e.stack.match(/:(\d+):/)?.[1] : '?')));
    ...
    console.log(`${b}: app=${info.appLen} | ${errors.slice(0,2).join(' || ')}`);
    await page.close();
  }
  await browser.close();
})();
