...
function register(deps) {
  ...
  // ---------- POST /api/auth/send-otp (Resend-based OTP delivery) ----------
  // Dedicated endpoint for Resend OTP delivery with template support
  handlers['POST /api/auth/send-otp'] = async (req, res) => {
    try {
      const ip = clientIp(req);
      if (!rateLimit('auth-send-otp:' + ip, 5, 15 * 60 * 1000)) {
        return sendJSON(res, { error: 'too many attempts' }, 429);
      }
      const body  = await readBody(req);
      const email = String(body.email || '').toLowerCase().trim();
      if (!isEmailStr(email)) return sendJSON(res, { error: 'invalid email format' }, 400);
      ...
      try {
        ...
        try {
          global.__CUSTOMER_OTPS_FALLBACK = global.__CUSTOMER_OTPS_FALLBACK || [];
          global.__CUSTOMER_OTPS_FALLBACK.push({
            email, code_hash: codeHash, expires_at: expiresAt, ip, created_at: new Date().toISOString()
          });
        } catch (_) {}
      }

      // Send OTP via email with Resend template
      const subject = 'Tu código de acceso';
      const html = `
        <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto;">
          <div style="background-color: #f5f5f5; padding: 20px; border-radius: 8px;">
            <h2 style="color: #333; margin-bottom: 20px;">Código de acceso</h2>
            <p style="color: #666; margin-bottom: 20px;">Tu código de acceso es:</p>
            <div style="background-color: #fff; border: 2px solid #e0e0e0; border-radius: 8px; padding: 20px; text-align: center;">
              <p style="font-size: 32px; font-weight: bold; letter-spacing: 8px; color: #000; margin: 0;">
                ${code}
              </p>
            </div>
            <p style="color: #999; font-size: 12px; margin-top: 20px;">
              Este código vence en 10 minutos. Si no solicitaste este código, ignora este correo.
            </p>
          </div>
        </div>
      `;
