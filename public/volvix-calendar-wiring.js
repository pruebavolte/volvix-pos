...
(function (global) {
    ...
    function getRoot() {
        // Opt-in only: render the calendar where the page explicitly placed
        // <div id="volvix-calendar-root"> or <div data-volvix-calendar>.
        // Never auto-inject at <body> end (was producing a stray calendar at the
        // bottom of every page that loaded this wiring).
        let root = document.getElementById('volvix-calendar-root')
            || document.querySelector('[data-volvix-calendar]');
        return root || null;
    }
    ...
    const Calendar = {
        ...
        render() {
            const root = getRoot();
            if (!root) return; // No explicit container on this page → don't render
            root.innerHTML = '';
    ...
    // Auto-init — but ONLY if the page explicitly placed a calendar container.
    // Pages that need the agenda do: <div id="volvix-calendar-root"></div>
    function maybeAutoInit() {
        const hasContainer = document.getElementById('volvix-calendar-root')
            || document.querySelector('[data-volvix-calendar]');
