...
// ESCENARIO 3: GET /api/admin/tenants devuelve tenants reales (acepta items o tenants)
test('3. /api/admin/tenants devuelve tenants reales', async () => {
  ...
  const list = j.items || j.tenants || [];
  console.log('  tenants count:', list.length);
  expect(list.length).toBeGreaterThanOrEqual(20);
  const sample = list[0];
  // Acepta slug TNT-XXX o UUID
  expect(sample.tenant_id || sample.id).toBeTruthy();
...
test('11. Sin token y token fake son rechazados', async () => {
  // Crear contexto sin baseURL para asegurar request crudo
  const noAuth = await request.newContext();
  const r1 = await noAuth.get(BASE + '/api/admin/tenants');
  const r2 = await noAuth.get(BASE + '/api/admin/tenants', {
    headers: { 'Authorization': 'Bearer fake.token.here' },
  });
  console.log('  sin token:', r1.status(), '· token fake:', r2.status());
  // Sin token DEBE ser 401 (unauthorized) — verificado manualmente con curl
