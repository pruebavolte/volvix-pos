...
**Marketplace en DOS PASOS (decision de Vicky, 2026-09-20):** hoy `https://github.com/pruebavolte/volvix-pos/releases/latest/download/VolvixPOS.apk` da **404** (el release Latest `v1.0.344` se publico sin APK) y la web se despliega al hacer merge a `main`, antes de que exista el primer tag con APK. Por eso:
1. **Merge inicial (`apk/loyverse`):** `public/marketplace.html` queda con el link **fijo a `v1.0.336`** (`.../releases/download/v1.0.336/VolvixPOS.apk`, que si existe); solo se quito el link roto a `VolvixCliente.apk`.
2. **Segundo commit, ya preparado, SIN merge:** rama `origin/apk/marketplace-latest` (un solo commit sobre `apk/loyverse` que cambia esa linea a `.../releases/latest/download/VolvixPOS.apk`). Se aplica (`git merge origin/apk/marketplace-latest` o `git cherry-pick` de ese commit) **solo despues del primer tag** y **solo si pasa la verificacion previa**:
   - `curl -sIL https://github.com/pruebavolte/volvix-pos/releases/latest/download/VolvixPOS.apk` termina en **HTTP 200** (no 404), y
   - `gh release view v1.0.N --repo pruebavolte/volvix-pos --json assets --jq '[.assets[].name]'` incluye `VolvixPOS.apk`.
   Si devuelve 404, NO aplicar: el job del APK del tag fallo o sigue corriendo (ver el paso 5 de arriba).
